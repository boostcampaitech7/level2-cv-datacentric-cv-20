...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir,
        ...
        if split == 'train': 
            self._lang_list = train_lang_list
        elif split == 'val':
...
class CORDDataset(Dataset):
    ...
    def _infer_dir(self, fname):
        lang_indicator = fname.split('.')[1]
        data_indicator = fname.split('_')[0]
...
class MergedDataset(Dataset):
    def __init__(self, root_dir,
        ...
        anno_path = osp.join(root_dir, f'{split}.json')
    ...
    def _infer_dir(self):
        return osp.join(self.root_dir, self.split)
    ...
    def __getitem__(self, idx):
        image_fname = self.image_fnames[idx]
        image_fpath = osp.join(self._infer_dir(), image_fname)
